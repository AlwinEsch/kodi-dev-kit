...
CJobWorker::CJobWorker(CJobManager *manager)/* : CThread("JobWorker")*/
{
  ...
  CreateThread(true); // start work immediately, and kill ourselves when we're done
}
...
void CJobManager::CancelJobs()
{
  ...
  while (m_workers.size())
  {
    lock.Unlock();
    m_jobEvent.Signal();
    usleep(0); // yield after setting the event to give the workers some time to die
    lock.Lock();
  }
}
...
bool CJobManager::IsProcessing(const CJob::PRIORITY &priority)
{
  CSingleLock lock(m_section);
...
int CJobManager::IsProcessing(const std::string &type)
{
  int jobsMatched = 0;
  CSingleLock lock(m_section);
...
CJob *CJobManager::GetNextJob(const CJobWorker *worker)
{
  ...
  while (m_running)
  {
    ...
    lock.Unlock();
    bool newJob = m_jobEvent.Wait(30000);
    lock.Lock();
    if (!newJob)
      break;
  }
